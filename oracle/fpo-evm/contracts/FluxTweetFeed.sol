 ...
 */
contract FluxTweetFeed is AccessControl, CLV2V3Interface {
    ...
    // time timestamp
    struct Transmission {
        string answer; // 192 bits ought to be enough for anyone
        uint64 timestamp;
    }
     ...
     */
    event NewTransmission(uint32 indexed aggregatorRoundId, string answer, address transmitter);
     ...
     */
    function latestTransmissionDetails() external view returns (string _latestAnswer, uint64 _latestTimestamp) {
     ...
     */
    function transmit(string _answer) external onlyRole(VALIDATOR_ROLE) {
